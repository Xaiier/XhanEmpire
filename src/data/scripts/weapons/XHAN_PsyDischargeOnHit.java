...
public class XHAN_PsyDischargeOnHit implements OnHitEffectPlugin {
    private static final float DISTANCE_THRESHOLD = 600f;
    private static final float DISTANCE_NO_FORCE_FALLOFF = 350f;
    private static final float ARC_EMP_DAMAGE = 900f;
    private static final float ARC_DAMAGE = 120f;
    ...
    private static final Color ARC_FRINGE_COLOUR_SHIP = new Color(121, 22, 235, 208);
    private static final Color ARC_FRINGE_COLOUR_MAGNET = new Color(121, 22, 235, 206);
    private static final Color ARC_CORE_COLOUR = new Color(248, 159, 177, 251);

    private static final Map<ShipAPI.HullSize, Float> BASE_FORCE_PER_HULLSIZE = new HashMap<>();

    static {
        BASE_FORCE_PER_HULLSIZE.put(ShipAPI.HullSize.FIGHTER, 1f);
        BASE_FORCE_PER_HULLSIZE.put(ShipAPI.HullSize.FRIGATE, 1f);
        BASE_FORCE_PER_HULLSIZE.put(ShipAPI.HullSize.DESTROYER, 1f);
        BASE_FORCE_PER_HULLSIZE.put(ShipAPI.HullSize.CRUISER, 1f);
        BASE_FORCE_PER_HULLSIZE.put(ShipAPI.HullSize.CAPITAL_SHIP, 1f);
    }
    ...
    private void spawnArcs(final Vector2f start, final ShipAPI source, final ShipAPI destination, float thickness, Color fringe, CombatEngineAPI engine) {
        ...
        for (int i = 0; i < num; i++) {
            engine.spawnEmpArcPierceShields(source, start, source, destination, DamageType.ENERGY, ARC_DAMAGE, ARC_EMP_DAMAGE, 3000f, "XHAN_PSYDISCHARGE_HIT_SOUND", thickness, fringe, ARC_CORE_COLOUR);
        }
    }
}
